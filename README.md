...
| 1 | 数据分析 | "分析一下销售数据，哪个地区销售额最高？" | 单步：excel\_summary.py |
...
│   ├── excel\_summary.py      #   Excel → 统计摘要 JSON（pandas）

│   ├── json\_to\_docx.py       #   摘要 JSON → Word 报告（python-docx）

│   ├── json\_to\_pptx.py       #   摘要 JSON → 汇报 PPT（python-pptx）

│   └── make\_test\_data.py     #   生成虚构测试数据
...
├── tests/run\_tests.py        # 10 个自动化测试用例（正常/边界/异常）

├── test\_data/                # 虚构测试数据（无真实个人信息）
...
{"id": "docx", "script": "scripts/json\_to\_docx.py",

&#x20;"args": {"input": "{{steps.summary.outputs.summary\_json}}", "output": "outputs/分析报告.docx"}}
...
python scripts\\excel\_summary.py --input test\_data\\销售数据.xlsx --output outputs\\摘要.json

python workflow.py workflows\\分析并生成Word报告和PPT.json --param excel=test\_data\\销售数据.xlsx
...
\- 失败即停：任一步骤失败立即终止流程并指明 failed\_step，不产生半成品输出。
